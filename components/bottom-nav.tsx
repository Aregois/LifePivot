...
import { Home, Calendar, ShoppingBag, User, Bot } from 'lucide-react'
...
export function BottomNav() {
    ...
    const { setShowMobileChat } = useEconomy()
    const { t } = useLanguage()
    ...
    return (
        <nav role="navigation" aria-label="Main navigation" className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full md:hidden z-50">
            ...
            <div className="relative h-[max(88px,88px+env(safe-area-inset-bottom,0px))] bg-[#141824] border-t border-white/[0.05] flex items-center justify-between px-6 pb-[env(safe-area-inset-bottom,0.5rem)] rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
                ...
                >
                    <div className="w-16 h-16 rounded-full bg-electric-blue border-4 border-[#0B0D17] flex items-center justify-center shadow-[0_0_20px_rgba(0,240,255,0.4)] cursor-pointer hover:bg-electric-blue/90 transition-colors z-10">
                ...
                {tabs.map((tab, idx) => {
                    ...
                    // Add extra margin to the middle items to make room for the central FAB
                    ...
                    return (
                        <Link
                            ...
                            className={`flex flex-col items-center justify-center w-14 h-full gap-1.5 transition-all active:scale-95 duration-200 ${isLeftCenter ? 'mr-4' : ''
                                } ${isRightCenter ? 'ml-4' : ''
