...
export function BottomNav() {
    ...
    return (
        <nav role="navigation" aria-label="Main navigation" className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full md:hidden z-50">
            ...
            <div className="relative h-[max(88px,88px+env(safe-area-inset-bottom,0px))] bg-[#141824] border-t border-white/[0.05] flex items-center justify-around px-2 pb-[env(safe-area-inset-bottom,0.5rem)] rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
                ...
                {tabs.map((tab, idx) => {
                    ...
                    return (
                        <Link
                            ...
                            className={`flex flex-col items-center justify-center min-w-[56px] h-full gap-1.5 transition-all active:scale-95 duration-200 ${isLeftCenter ? 'mr-4' : ''
                                } ${isRightCenter ? 'ml-4' : ''
